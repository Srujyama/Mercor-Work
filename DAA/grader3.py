#!/usr/bin/env python3
"""
Gemini 3.0 Dual Autograder (Text + File/Image) for Eval View Only
-----------------------------------------------------------------
...
Inputs:
    ...
    - File/Image solution: "Gemini 3.0 Pro Response (File Output)"
...
# IMPORTANT: Gemini 3.0 model name (no "models/" prefix for SDK)
# Override via GEMINI_MODEL env if needed, e.g. "gemini-3-pro-preview"
...
GEM_SCORE_FIELD = "Gemini 3.0 Autorater - Gemini 2.5 Response Score"
GEM_SUMMARY_FIELD = "Gemini 3.0 Autorater - Gemini 2.5 Response Summary"


MAX_RETRIES = 4

# Cache directory for Gemini file uploads
CACHE_DIR = Path("./gemini_cache_3_0")
...
async def with_retries(fn):
    ...
    Retries everything except hard INVALID_ARGUMENT / 400 errors.
    """
...
def download_attachment(att, max_bytes=20_000_000):
    """
    Download a single Airtable attachment to disk with caching.
    Returns Path or None if download fails or file too large.
    """
    url = att.get("url")
    if not url:
        return None
    ...
    try:
        ...
        if len(content) > max_bytes:
            logger.warning(
                f"Attachment {att.get('filename')} too large ({len(content)} bytes), skipping."
            )
            return None
...
class Gemini30Autograder:
    def __init__(self):
        # Airtable client
        self.air = Api(AIRTABLE_API_KEY).table(AIRTABLE_BASE_ID, AIRTABLE_TABLE_ID)

        # Gemini client (3.0-capable)
        # You can tweak api_version if your environment requires it, but
        # default should work for most setups.
    ...
    async def grade(self, prompt, rubric, text_solution, attachments):
        ...
        user_prompt = (
            "Grade the model's response according to the rubric.\n\n"
            f"ORIGINAL PROMPT:\n{prompt}\n\n"
            f"{text_section}"
            f"CRITERIA (JSON mapping key -> description):\n{json.dumps(crit_map)}\n\n"
            f"ATTACHMENTS (metadata only):\n{attachment_meta}\n\n"
            "Return ONLY the required JSON object as specified in the system instructions."
        )

        # ---- upload attachments to Gemini (if any) ----
        uploaded_parts = []
        ...
        # Build parts: text prompt + uploaded files
    ...
    async def process_record(self, rec):
        ...
        try:
            ...
            self.stats["graded"] += 1
            logger.info(f"{rec_id}: ✔ updated (score={pct:.2f})")
        except Exception as e:
