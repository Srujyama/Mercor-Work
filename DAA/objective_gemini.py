...
def extract_pass_flags_with_weight(summary_json: Any) -> List[Tuple[bool, str]]:
    ...
    NOW MATCHES overall_agreement PASS LOGIC:

      - We only count a criterion if BOTH autorating and human_rating
        are explicitly booleans.
      - pass = (autorating == human_rating)
      - Otherwise: skip
    """
    ...
    for crit in summary_json:
        ...
        # ✅ Same rule as overall_agreement: require both booleans
        if not (isinstance(autorating, bool) and isinstance(human_rating, bool)):
            continue
