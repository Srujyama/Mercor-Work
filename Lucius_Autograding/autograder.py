#!/usr/bin/env python3
"""
Multi-turn GPT-5 Autograder - Lucius EDU (batched + verbose, fixed, parallelized)
...
- Parallel grading across inputs within a record; per-key + global API semaphores
"""
...
class MultiTurnGPT5Autograder:
    def __init__(self):
        ...
        # Per-key concurrency + global governor
        self.per_key_semaphores = [
            asyncio.Semaphore(max(1, PER_KEY_PARALLEL)) for _ in self.clients
        ]
        self.global_api_semaphore = asyncio.Semaphore(
            max(2, PER_KEY_PARALLEL * len(self.clients))
        )
    ...
    async def _pick_client(self):
        ...
        return self.clients[idx], self.per_key_semaphores[idx]
    ...
    async def _chat_json(self, messages):
        ...
        """
        client, key_sem = await self._pick_client()

        async def _call_json():
            async with self.global_api_semaphore, key_sem:
    ...
    async def grade_single_criterion(
        ...
        try:
            ...
            if m:
                idx = next(
                    (i for i, ln in enumerate(lines) if "FINAL_DECISION" in ln.upper()),
                    -1,
                )
    ...
    async def process_record(self, record: dict) -> bool:
        ...
        # Build tasks for all columns that need grading
        tasks = []
        col_names = []
        for col in EVAL_INPUT_FIELDS:
            ...
            tasks.append(self.grade_solution(text, rubric, prompt))

        if not tasks:
            return False

        # Run all column gradings concurrently (bounded by semaphores inside grade_solution)
        results = await asyncio.gather(*tasks, return_exceptions=True)

        # Coalesce updates for this record in a single Airtable call
        updates, graded_any = {}, False
        for col, res in zip(col_names, results):
            out_score = score_field_name(col)
            out_summary = summary_field_name(col)
            if isinstance(res, Exception):
                logger.error(f"Grading failed for {record_id} / {col}: {res}")
                continue
            updates[out_score] = res["percentage"]
            updates[out_summary] = res["summary"]
            graded_any = True
    ...
    async def run(self):
        logger.info(
            "🤖 Starting Multi-turn GPT-5 Autograder (batched+verbose, fixed, parallelized)"
        )
        ...
        # Cap total in-flight records; per-key limits still apply inside
        overall_limit = min(16, max(4, len(self.clients) * 3))
