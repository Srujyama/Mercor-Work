...
FIELD_TASK_ID = os.getenv("FIELD_TASK_ID", "Task ID")  # ← filter key for targeted re-runs

# Optional: comma/space-separated list of Task IDs to run (fallback to CLI flag)
...
def fetch_airtable_records(base_id: str, table: str, view: str) -> List[Dict[str, Any]]:
    ...
    params = base_params | {
        "fields[]": [FIELD_ATTACHMENTS, FIELD_PROMPT, FIELD_CONTEXT, FIELD_TASK_ID]
    }
    ...
    def _drain(params_: Dict[str, Any]):
        ...
        while True:
            r = SESSION.get(url, headers=headers, params=local_params, timeout=REQUEST_TIMEOUT)
...
def _download_one(url: str, dest_dir: Path) -> Optional[Path]:
    ...
    if not filename:
        filename = (url.split("?")[0].split("/")[-1] or f"file-{sha256_of_bytes(r.content)}")
...
def collect_images_from_attachments(
    ...
    if attachments and isinstance(attachments, list) and len(attachments) == 1 and isinstance(attachments[0], list):
...
def upload_files(client: genai.Client, image_paths: List[Path]) -> List[Any]:
    ...
    def _upload_one(p: Path):
        ...
        if cached and cached.get("file_uri"):
            ...
            return types.File(name=cached["file_uri"])  # SDK accepts file ref by name/uri
        fh = client.files.upload(path=p.as_posix())
        UPLOADS_CACHE[sha] = {"file_uri": getattr(fh, "name", None) or getattr(fh, "uri", None)}
        return fh

    with ThreadPoolExecutor(max_workers=min(8, MAX_WORKERS)) as ex:
        futs = {ex.submit(_upload_one, p): p for p in image_paths}
        for fut in tqdm(as_completed(futs), total=len(futs), desc="Uploading to Gemini"):
...
def generate_many(
    ...
    def _one_call(i: int):
        try:
            ...
            return {"run_label": "web_on" if web_on else "web_off", "iteration": i, "output_text": out_text}
        except Exception as e:
            return {"run_label": "web_on" if web_on else "web_off", "iteration": i, "error": repr(e)}
...
def process_record(client: genai.Client, record: Dict[str, Any], base_out_dir: Path) -> Dict[str, Any]:
    ...
    images = collect_images_from_attachments(attachments, work_dir) if attachments else []
    ...
    off_results = generate_many(client, GEMINI_MODEL, text_prompt, file_handles, RUNS_PER_MODE, web_on=False)
    on_results = generate_many(client, GEMINI_MODEL, text_prompt, file_handles, RUNS_PER_MODE, web_on=True)
    ...
    (work_dir / "result.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    for i in range(RUNS_PER_MODE):
        ...
        if i < len(off_results):
            r = off_results[i]
            payload[label] = (r.get("output_text") or f"ERROR: {r.get('error', 'unknown')}")[:50000]
    for i in range(RUNS_PER_MODE):
    ...
    return {"id": rec_id, "fields": payload, "errors": [r for r in meta["runs"] if "error" in r]}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Airtable → Gemini pipeline")
    parser.add_argument(
        "--task-ids",
        nargs="*",
        default=None,
        help="One or more Task IDs to (re)run. Accepts multiple args or a single comma/space-separated string.",
    )
    parser.add_argument(
        "--record-ids",
        nargs="*",
        default=None,
        help="(Optional) One or more Airtable record IDs (recXXXX) to (re)run.",
    )
    args = parser.parse_args()

    # Normalize task IDs from CLI or env
    task_ids: List[str] = []
    if args.task_ids:
        # support: --task-ids A B C  or  --task-ids "A,B,C"
        tmp: List[str] = []
        for item in args.task_ids:
            tmp.extend(item.replace(",", " ").split())
        task_ids = [t.strip() for t in tmp if t.strip()]
    elif TASK_IDS_ENV:
        task_ids = [t.strip() for t in TASK_IDS_ENV.replace(",", " ").split() if t.strip()]

    record_ids: List[str] = []
    if args.record_ids:
        tmp: List[str] = []
        for item in args.record_ids:
            tmp.extend(item.replace(",", " ").split())
        record_ids = [r.strip() for r in tmp if r.strip()]
    ...
    records = fetch_airtable_records(AIRTABLE_BASE_ID, AIRTABLE_TABLE_NAME, AIRTABLE_VIEW_NAME)
    print(f"Found {len(records)} record(s) before filtering")

    # Filter by Task ID and/or record ID if requested
    if task_ids:
        ti = set(task_ids)
        records = [r for r in records if str(r.get("fields", {}).get(FIELD_TASK_ID, "")) in ti]
        print(f"Filter: Task IDs={sorted(ti)} → {len(records)} record(s)")

    if record_ids:
        ri = set(record_ids)
        records = [r for r in records if r.get("id") in ri]
        print(f"Filter: Record IDs={sorted(ri)} → {len(records)} record(s)")
    ...
    with ThreadPoolExecutor(max_workers=min(MAX_WORKERS, 12)) as ex:
        futs = {ex.submit(process_record, client, rec, OUTPUT_DIR): rec.get("id") for rec in records}
    ...
    (OUTPUT_DIR / "run_summary.json").write_text(json.dumps(summaries, ensure_ascii=False, indent=2), encoding="utf-8")
