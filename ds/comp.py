...
Test mode:
- Only processes the first 10 records that need grading.
"""
...
# How many Airtable records to process in parallel
...
class CombinedDualAutograder:
    ...
    async def run(self):
        ...
        # 🔹 TEST RUN: only first 10 records
        records = records[:10]
        total = len(records)
        logger.info(f"[Combined] TEST RUN on {total} records")
        ...
        with tqdm(total=total, desc="Grading records (test 10)") as pbar:
