...
# ---- Collect multiple Gemini keys (unique only) ----
gemini_keys: List[str] = []
base_key = os.getenv("GOOGLE_API_KEY")
if base_key:
    gemini_keys.append(base_key)
...
# User said there are 6 keys, but two are duplicates; we keep uniques only (5)
if not gemini_keys:
    raise RuntimeError("Missing any GOOGLE_API_KEY[_N] in .env")

# Enforce using at most 5 unique keys as requested
if len(gemini_keys) > 5:
    gemini_keys = gemini_keys[:5]

GEMINI_MODEL = os.getenv("GEMINI_MODEL", "gemini-3-pro-preview")
GPT_MODEL = os.getenv("OPENAI_MODEL", "gpt-5")

GPT_KEY_CONCURRENCY = int(os.getenv("GPT_KEY_CONCURRENCY", "5"))
# This is *per key* concurrency now
GEMINI_KEY_CONCURRENCY = int(os.getenv("GEMINI_KEY_CONCURRENCY", "15"))
...
class CombinedDualAutograder:
    def __init__(self):
        ...
        # ---- Gemini: multiple clients, one per API key ----
        self.gem_clients: List[genai.Client] = []
        self.gem_client_sems: List[asyncio.Semaphore] = []
        for idx, key in enumerate(gemini_keys):
            client = genai.Client(
                api_key=key,
                http_options=types.HttpOptions(api_version="v1beta"),
            )
            self.gem_clients.append(client)
            self.gem_client_sems.append(asyncio.Semaphore(GEMINI_KEY_CONCURRENCY))
            logger.info(f"[GEMINI] Initialized client {idx} with its own semaphore.")

        if not self.gem_clients:
            raise RuntimeError("No valid Gemini API clients initialized.")

        # Global semaphore: cap overall concurrency (GPT + Gemini)
        total_gem_conc = GEMINI_KEY_CONCURRENCY * len(self.gem_clients)
        global_limit = max(4, GPT_KEY_CONCURRENCY + total_gem_conc)
        self.global_sem = asyncio.Semaphore(global_limit)
        self.gpt_sem = asyncio.Semaphore(GPT_KEY_CONCURRENCY)

        # round-robin index for Gemini client selection
        self._gem_rr_index = 0
        self._gem_rr_lock = asyncio.Lock()
    ...
    async def _pick_gem_client(self) -> Tuple[genai.Client, asyncio.Semaphore]:
        """Round-robin pick of a Gemini client + its semaphore."""
        async with self._gem_rr_lock:
            idx = self._gem_rr_index % len(self.gem_clients)
            self._gem_rr_index += 1
        return self.gem_clients[idx], self.gem_client_sems[idx]
    ...
    async def _gemini_chat_json(self, messages):
        ...
        async def _call_with_client(client, sem):
            async with self.global_sem, sem:
        ...
        client, sem = await self._pick_gem_client()
        resp = await _with_retries(lambda: _call_with_client(client, sem))
    ...
    async def grade_files_with_gemini(
        ...
        # Pick a single Gemini client for this whole grading (uploads + generate)
        client, client_sem = await self._pick_gem_client()
        ...
        async def handle_attachment(att: Dict[str, Any]):
            ...
            async with self.global_sem, client_sem:
                try:
