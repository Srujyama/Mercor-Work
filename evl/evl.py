...
def call_gemini(
    ...
    max_retries: int = 3,
    timeout_s: int = GEMINI_MAX_SECONDS,
) -> Tuple[str, Optional[str]]:
    """
    Call Gemini with text + optional files.
    uploaded_files: list of (file_obj, mime_type)

    NOTE: We pass a per-request timeout via request_options. If your installed
    google-genai version does not support this kwarg, remove it or adapt.
    """
    guard_rails = (
    ...
    raise RuntimeError(
        f"Gemini call failed after {max_retries} attempts (or timed out at {timeout_s}s): {last_err}"
    )
...
def process_view(
    ...
    def _gen_task(rec_id: str, prompt: str, attachments: List[Dict[str, Any]]):
        ...
        # now call with timeout
        text, served = call_gemini(
            client,
            model_name,
            prompt,
            uploaded_files=uploaded,
            timeout_s=GEMINI_MAX_SECONDS,
        )
