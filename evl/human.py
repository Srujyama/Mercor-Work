...
def extract_criterion_agreements(
    summary_json: Any,
) -> List[Tuple[str, bool]]:
    """
...
def compute_stats_for_criterion_types(
    ...
    (criterion_type, model), where model ∈ {Gemini, GPT5}.

    Agreement is defined as autorating == human_rating.
    """
    ...
    # We'll build raw 0/1 lists per (model, criterion_type)
    # structure: data[model_name][criterion_type] = [0/1, ...]
    ...
    for _, r in sub.iterrows():
        ...
        # Gemini
        for ct, passed in extract_criterion_agreements(g_summary):
            data["Gemini 3.0"][ct].append(1.0 if passed else 0.0)

        # GPT-5
        for ct, passed in extract_criterion_agreements(p_summary):
            data["GPT-5"][ct].append(1.0 if passed else 0.0)

    # Now aggregate per (model, criterion_type)
    for model_name, by_type in data.items():
        for ct in CRITERION_TYPES:
            ...
            # scale to percentage out of 100
...
def plot_criterion_type_stats(stats_df: pd.DataFrame, domain: str, output_path: str):
    """
    Plot criterion-type agreement chart for a given domain.
    """
    if stats_df.empty:
    ...
    if domain == "education":
        title = "Criterion-Type Agreement – Education"
    elif domain == "hle":
        title = "Criterion-Type Agreement – HLE"
    else:
        title = f"Criterion-Type Agreement – {domain.capitalize()}"
    ...
    logger.info(
        "Saved human vs criterion-type chart for domain '%s' to %s", domain, output_path
    )
